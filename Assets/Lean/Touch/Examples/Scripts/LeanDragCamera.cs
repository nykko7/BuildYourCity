...
namespace Lean.Touch
{
	...
	public class LeanDragCamera : MonoBehaviour
	{
		...
		protected virtual void LateUpdate()
		{
			...
			// Se mueve sin pasar los limites del mapa:
			if (transform.position.x <= 39 || transform.position.x >= 122) {
				remainingDelta.x = 0;
				newRemainingDelta.x = 0;				
			}
			if (transform.position.z >= 161 || transform.position.z <= 60){
				remainingDelta.z = 0;
				newRemainingDelta.z = 0;
			}	
